...
class EasyLog:
    ...
    def _setup_logger(self):
        ...
        # 配置日志
        self._logger = logging.getLogger("EasyLog")
    ...
    def _update_handlers(self):
        ...
        logging 模块内置的文件滚动功能不足以实现 easylog 的滚动需求，因此我们自己来实现。
...
class OutputList(list):
    ...
    def append(self, item):
        ...
        """
        if item in self:
            print(f"Easylog warning: An existing path was added: <{item}>")
            return 1
        else:
            print(f"Easylog debug: Path added successfully: <{item}>")
        super().append(item)
...
def new(log_path="", config_path=""):
    return EasyLog(log_path=log_path, config_path=config_path)
